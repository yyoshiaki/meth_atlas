...
class Deconvolve:
    def __init__(self, atlas_path, samp_path, out_dir, resid, wgbs, slim=False, plot=False):
        ...
        self.wgbs = wgbs                            # WGBS mode
        self.out_bname = self.get_bname(samp_path)  # output files path w/o extension

        # Load input files:
        self.atlas = self.load_atlas(atlas_path, wgbs)    # Atlas
        self.samples = self.load_sample(samp_path, wgbs)  # Samples to deconvolve
    ...
    @staticmethod
    def _validate_csv_file(csv_path, wgbs):
        """
        ...
        # take a peek and validate the file format
            ...
            elif (not str(input_head.iloc[0, 0]).startswith('cg')) & (not wgbs): 
    ...
    def load_sample(self, samp_path, wgbs):
        """
        ...
        # validate path:
        Deconvolve._validate_csv_file(samp_path, wgbs)
...
def main():
    ...
    parser.add_argument('--wgbs', action='store_true',
                        help='Switch to WGBS mode.\n'
                        'The first columns must be chrom:pos (e.g chr14:68790171)')
